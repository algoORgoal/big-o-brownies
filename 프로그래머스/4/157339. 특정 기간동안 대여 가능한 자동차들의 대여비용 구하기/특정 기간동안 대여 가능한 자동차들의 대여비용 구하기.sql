    ...
    HAVING
        COUNT(*) = COUNT(
            CASE
                ...
                WHEN END_DATE < '2022-11-01' OR '2022-11-30' < START_DATE THEN 1
            END
        )
),
ON_SALE_CAR_INFO AS (
    SELECT
        ...
        CASE
            WHEN discount.DURATION_TYPE = '7일 이상' THEN FLOOR(available.DAILY_FEE * (1 - 0.01 * discount.DISCOUNT_RATE) * 30)
            WHEN discount.DURATION_TYPE = '30일 이상' THEN FLOOR(available.DAILY_FEE * (1 - 0.01 * discount.DISCOUNT_RATE) * 30)
            ELSE available.DAILY_FEE * 30
        END AS FEE
    FROM AVAILABLE_CAR_INFO available
    LEFT OUTER JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN discount
    ON available.CAR_TYPE = discount.CAR_TYPE
),
AFFORDABLE_CAR_INFO AS (
